...
2. **Certiva → Templates → Add New**: upload a background — an image (JPG/PNG) or a single-page PDF, exactly one of either type — choose a page size/orientation, then use the visual designer to drag the built-in placeholder fields (student name, college, event title, event date, certificate ID, issue date) onto the certificate, styling each one (font, size, color, alignment, bold/italic). You can also add a custom placeholder tied to a student's extra field, or static text. A PDF background is shown in the canvas using your browser's own PDF viewer as a visual guide — alignment with it isn't pixel-perfect (browsers vary in how they render an embedded PDF), so **Preview with Sample Data** — which renders a real PDF through the same pipeline used for actual certificates — is the accurate way to check a layout before saving.
...
**PDF backgrounds:** a template's background can be a PDF instead of an image. This uses mPDF's bundled FPDI integration (`setasign/fpdi`, already a transitive `mpdf/mpdf` dependency — no extra library to install) to import page 1 of the source PDF and stretch it to fill the certificate page, with the positioned text fields written on top exactly as with an image background.
